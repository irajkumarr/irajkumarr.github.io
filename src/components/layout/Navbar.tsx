...
import { Menu, X, FileText, ArrowUpRight, Terminal as TerminalIcon } from "lucide-react";
...
export function Navbar() {
  ...
  return (
    <>
      ...
      >
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-14">
            ...
            {/* Actions: Terminal Button + Socials + Theme + Mobile Button */}
            <div className="flex items-center gap-2">
              {/* Terminal Launcher Button */}
              <button
                type="button"
                onClick={() => setTerminalOpen(true)}
                className="inline-flex items-center gap-1.5 h-9 px-2.5 sm:px-3 rounded-xl border border-surface-border bg-surface/80 hover:bg-surface-hover hover:border-accent/50 text-foreground transition-all duration-200 shadow-2xs group"
                aria-label="Open Interactive Developer Terminal"
                title="Launch Interactive Terminal"
              >
                <div className="p-1 rounded bg-accent/15 text-accent group-hover:scale-105 transition-transform">
                  <TerminalIcon className="w-3.5 h-3.5" />
                </div>
                <span className="font-mono text-xs font-medium hidden sm:inline text-foreground/90">
                  Terminal
                </span>
                <span className="text-[10px] font-mono text-accent hidden lg:inline px-1 py-0.2 rounded bg-accent/10 border border-accent/20">
                  cmd
                </span>
              </button>
          ...
          <div className="md:hidden border-b border-surface-border/90 bg-background/95 backdrop-blur-xl px-5 pt-3 pb-6 space-y-3 animate-fade-in shadow-xl">
            ...
            <div className="pt-3 border-t border-surface-border flex flex-col gap-3">
              <button
                ...
                className="w-full flex items-center justify-center gap-2 py-2 px-3 rounded-xl border border-surface-border bg-surface/80 hover:border-accent/50 text-xs font-mono text-foreground font-semibold shadow-2xs"
              >
      ...
      {/* Pop-up Developer Terminal Modal supporting Dark & Light mode */}
      <TerminalModal
        isOpen={terminalOpen}
